...
class Users(db.Model):
    ...
    countryname = db.Column(db.String(32), unique=True)
    accountnum = db.Column(db.String(10), unique=True)
    password = db.Column(db.String(64))
    pin = db.Column(db.String(4))
    balance = db.Column(db.Float())

    def __init__(self, accountnum, password, countryname, pin):
...
if __name__ == "__main__":
	app.run(host=app.config['LISTENADDR'],port=1337)
